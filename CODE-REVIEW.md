...
| 6 | ✅ **Done 2026-06-10** — Astronaut lazy-loads behind an in-view gate (`useInView`, 600px rootMargin): Home chunk 850 → 209 kB, the 642 kB three.js scene downloads only on approach (initial JS −55%). Duplicate CDN Bootstrap 5.0.1 + unused Font Awesome removed from index.html (bundled 5.1.3 is the single copy; full Bootstrap replacement deferred). Dropped axios, web-vitals, @testing-library/*, @babel/plugin-proposal-private-property-in-object. ARIA: half-tablist replaced with honest toggle-button group (`aria-pressed`; CategoryButton had been silently dropping the role props anyway). Bonus: §5f's body-as-scroll-container quirk was quirks-mode fallout from the missing doctype — resolved by the step-4 doctype fix; the viewport scrolls natively now. | 0.5–1 day | faster first paint, honest a11y |
| 7 | ✅ **Done 2026-06-10** (asset half) — `public/` 72 → 30 MB: 35 orphaned files (20.4 MB — Bild1-7, Performance/*, header variants, an 8 MB GIF, a stray `.zshrc`) moved to gitignored `_archive/public-orphans/`; posters PNG→JPG (~5.8 MB → ~0.7 MB), heavy PNGs→WebP (Design, NextjsPortfolio, Hustle, Badges), Nature/hero re-encoded, `Video13.mp4` 6.2→0.9 MB and `space-studio-tour.mp4` 9.0→3.4 MB (crf 28-30). All refs updated; the referenced-media-exists test pins it. **CI intentionally skipped** — a Forgejo Actions pipeline already runs on the DGX Spark; the open issue there is the Forgejo→Strato deploy mapping, not the pipeline. The §8 data tests run locally via `npm test` and can be dropped into that workflow as one step. | 0.5 day | regression safety net |
