...
| 4 | **CRA → Vite (+ vitest)**: `.js→.jsx` renames, `process.env.REACT_APP_*` → `import.meta.env.VITE_*` (`PUBLIC_URL` in `portfolioData.js`/`Header.jsx`), index.html move | 1–2 days | kills the 68-vuln audit, ~10× dev-server speed, removes EOL platform |
| 5 | Data layer: split `portfolioData.js` (1,766 LOC) into per-project modules; pick **one** translation strategy (all keys or all literals) instead of the current mix bridged by `getTranslatedProject` | ~1 day | editing a project stops being a scroll hunt |
