...
- **Tier 1, flagship feature cards (5):** `ornith-optimization`, `dflash-retrain`, `belay`, `quantum-performance`, `nutrition-rag-autonomous` get `flagship: true` (cap 2-6, enforced by a data-integrity test in `portfolioData.test.js`, not by comment; review finding P2; nutrition promoted in design round 5).
